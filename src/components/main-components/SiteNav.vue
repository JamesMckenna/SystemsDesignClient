<template>
    <div id="animateNavWithThisDiv">
        <nav id="mainNav" class="mainNavAnimation">
            ...
            <div id="navContainer">
                <div id="mainNavBtn">
                    <button id="navBtn" title="Open Close nav button" aria-label="Open Close nav button" type="button"><span role="img" aria-label="[Nav button image]" title="Nav button image" id="mainNavBtnImg"></span></button>
                </div>
                <div id="wrapTitle">
...
export default class SiteNav extends Vue {
    ...
    mq: MediaQueryList = {} as MediaQueryList;
    navLinksWrap: HTMLHtmlElement = {} as HTMLHtmlElement;
    loginBtn: HTMLHtmlElement = {} as HTMLHtmlElement;
    login: HTMLHtmlElement = {} as HTMLHtmlElement;
    navLinks: HTMLHtmlElement = {} as HTMLHtmlElement;
    ...
    * Hide/Show nav bar
    */
    scrollStopped(evt: Event): void {
        let nav = document.getElementById('animateNavWithThisDiv')!;
        let sy = window.scrollY;
        let timer: number = -1;
        //if login open, close it
        if (this.login.classList.contains('loginIsOpen')) {
            this.openLogin(evt);
        }
        //if nav bar open, close it
        if (this.navLinks.classList.contains('mainNavIsOpen')) {
            this.mainNavBtn(evt);
        }
        //Hide up animation
        if (sy > 80) {
        ...
        timer = window.setTimeout(() => {
            this.scrollCurrentStopped = window.scrollY;
            this.scrollLastStopped = this.showHideNav(this.scrollCurrentStopped);
            
        }, 300);      
    }

    showHideNav(scrollCurrentStopped: number) : number {  
    ...
    */
    resizeListener(evt: Event): void {
        let newParent: HTMLHtmlElement;
        let loginBtn: HTMLHtmlElement;

        let logoImg: HTMLHtmlElement = <HTMLHtmlElement>document.getElementById('logo')!;

        //if less than 40em (640px)
        if ((!this.mq.matches) && (this.login.hasAttribute('data-mobile'))) {
            loginBtn = <HTMLHtmlElement>document.getElementById('loginBtn')!;
            this.login.removeAttribute('data-mobile');
            newParent = this.navLinks;

            //move logo 
            let loginLinks = <HTMLHtmlElement>document.getElementById('loginLinks')!;
            loginLinks.appendChild(logoImg);

            //remove links wrapper div for cleaner layout
            this.navLinksWrap.style.display = 'none';
        ...
        if ((this.mq.matches) && !(this.login.hasAttribute('data-mobile'))) {
            loginBtn = <HTMLHtmlElement>document.getElementById('loginBtn')!;
            this.login.setAttribute('data-mobile', 'false');

            newParent = <HTMLHtmlElement>document.getElementById('loginLinks')!;

            //move logo 
            const wrapTitle: HTMLHtmlElement = <HTMLHtmlElement>document.getElementById('wrapTitle')!;
    ...
    */
    mainNavBtn (evt: Event): void {
        ...
        if (this.navLinks.getAttribute('data-ndd') != 'closed') {
            let currentlyOpen = this.navLinks.getAttribute('data-ndd');
            let close = document.getElementById(currentlyOpen!)!;
            close.style.display = 'none';
            this.navLinks.setAttribute('data-ndd', 'closed');
        }

        if (this.navLinks.classList.contains('mainNavIsOpen') === true) {
            //start css animation
            this.navLinks.className = 'mainNavIsClosed';
        }
        else {
            this.navLinksWrap.style.display = 'flex';
            //start css animation
            this.navLinks.className = 'mainNavIsOpen';
        }

        //change css property style='display: "display-type" '
        this.navLinks.addEventListener('animationend', (evt) => {
            if (this.navLinks.classList.contains('mainNavIsClosed')) {
                this.navLinksWrap.style.display = 'none';
            }
        }, true);
        evt.stopPropagation();
    ...
    */
    openLogin(evt: Event): void {
        //if main nav open, close it
        if (this.navLinks.classList.contains('mainNavIsOpen') && (this.login.hasAttribute('data-mobile'))) {
            this.mainNavBtn(evt);
        }

        const displayState = this.login.classList.contains('loginIsClosed');
        if (displayState) {
            this.login.style.display = 'block';
            //start css animation
            this.login.className = 'loginIsOpen';
        } else {
            //start css animation
            this.login.className = 'loginIsClosed';
        }
        //change css property style='display: "display-type" '
        this.login.addEventListener('animationend', (evt) => {
            if (this.login.classList.contains('loginIsClosed')) {
    ...
    */
    nddOpenClose(evt: Event): void {
        // <a> that activates open/close 
        const target: HTMLHtmlElement = <HTMLHtmlElement>evt.target;
        // <div> element we actually want to target
        const sibling: HTMLHtmlElement = <HTMLHtmlElement>target.nextElementSibling || null;

        // return if not ndd link that was clicked
        if (sibling === null) {
            return;
        } else {
            const nddId: string = <string>sibling.getAttribute('id');
            const ndd: HTMLHtmlElement = <HTMLHtmlElement>document.getElementById(nddId);
            let currentlyOpen = this.navLinks.getAttribute('data-ndd');
            ...
            if (ndd === null) {
                ...
                this.navLinks.setAttribute('data-ndd', nddId);
    ...
    mounted(){
        ...
        window.addEventListener('scroll', this.scrollStopped, true);

        this.mq = window.matchMedia('(min-width: 40em)');
        window.addEventListener('load', this.resizeListener, true);
        window.addEventListener('resize', this.resizeListener, true);

        this.mainNav = <HTMLHtmlElement>document.getElementById('mainNavBtn');
        this.mainNav.addEventListener('click', this.mainNavBtn, true);

        this.navLinksWrap = <HTMLHtmlElement>document.getElementById('navLinksWrap');
        this.navLinksWrap.addEventListener('click', this.mainNavBtn, false);

        this.navLinks = <HTMLHtmlElement>document.getElementById('navLinks');
        this.navLinks.addEventListener('click', this.nddOpenClose, false);

        this.loginBtn = <HTMLHtmlElement>document.getElementById('loginBtn');
        this.loginBtn.addEventListener('click', this.openLogin, false);

        this.login = <HTMLHtmlElement>document.getElementById('login')!;
    }
}
</script>>
...
@keyframes mainNavAnimation {
    from {
        -webkit-box-shadow: .0625rem .0625rem .625rem .1875rem rgb(105,105,134) inset;
        -moz-box-shadow: .0625rem .0625rem .625rem .1875rem rgb(105,105,134) inset;
        box-shadow: .0625rem .0625rem .625rem .1875rem rgb(105,105,134) inset;
    }

    to {
        -webkit-box-shadow: 0rem 0rem 0rem -.025rem rgb(105,105,134) inset;
        -moz-box-shadow: 0rem 0rem 0rem -.025rem rgb(105,105,134) inset;
        box-shadow: 0rem 0rem 0rem -.025rem rgb(105,105,134) inset;
    }
}
...
#login > div > a:hover, a:hover {
    color: var(--a-hover-colour);
    font-weight: normal;
}
#navBtn:hover, #loginBtn:hover, li:hover {
    background: -moz-linear-gradient(left, var(--navlink-gradient-hov-1) 0%, var(--navlink-gradient-hov-2) 50%, var(--navlink-gradient-hov-3) 100%);
    background: -webkit-linear-gradient(left, var(--navlink-gradient-hov-1) 0%, var(--navlink-gradient-hov-2) 50%, var(--navlink-gradient-hov-3) 100%);
    background: linear-gradient(to right, var(--navlink-gradient-hov-1) 0%, var(--navlink-gradient-hov-2) 50%, var(--navlink-gradient-hov-3) 100%);
    filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#00000000', endColorstr='#99000000',GradientType=1 );
}
#navBtn:active, #loginBtn:active, li:active {
    background: -moz-linear-gradient(left, var(--navlink-gradient-active-1) 50%, var(--navlink-gradient-active-2) 100%); /* FF3.6-15 */
    background: -webkit-linear-gradient(left, var(--navlink-gradient-active-1) 50%, var(--navlink-gradient-active-2) 100%); /* Chrome10-25,Safari5.1-6 */
    background: linear-gradient(to right, var(--navlink-gradient-active-1) 50%, var(--navlink-gradient-active-2) 100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
...
i {
    border: solid black;
    border-width: 0 .0325rem .0325rem 0;
    margin-left: .5rem;
    margin-top: .25rem;
    margin-bottom: .25rem;
...
@media (min-width: 20rem) {
    ...
    #wrapTitleH1 {font-size: 1.625rem;}

    #logoFlip {display: none;}
